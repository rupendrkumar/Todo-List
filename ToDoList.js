...
const taskCOunter = document.getElementById('display');
const addbutton = document.getElementById('new-button');

//used to add tasks to browser page
...
function toggleTask(taskId){
    ...
    if(toggleTasks.length>0){
        ...
        else if(document.getElementById('done').style.color=='black'){
            renderCompleteList();
        }
        return;
    }
...
function typing(){
    if(addTaskInput.value!=""){
        addbutton.classList.replace('add','new-button-active');
    }else{
        addbutton.classList.replace('new-button-active','add');
    }
}
...
function handleClickListener(e){
    ...
    if(target.className == 'delete fa-solid fa-trash'){
        ...
        document.getElementById('all-tasks').style.color = 'grey';
        document.getElementById('done').style.color = 'grey';
        document.getElementById('uncompleted').style.color = 'black';
        renderUncompleteList();
        return;
    }
    else if(target.id == 'all-tasks'){
        document.getElementById('all-tasks').style.color = 'black';
        document.getElementById('done').style.color = 'grey';
